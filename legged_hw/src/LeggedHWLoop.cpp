...
// The complete controller loop, about joints.
void LeggedHWLoop::update() {
  ...
  // Sleep
  const auto sleepTill = currentTime + std::chrono::duration_cast<Clock::duration>(desiredDuration); // compare la durée du loop avec la loop_frequency ??
  usleep(10000); // TODO : vérifier l'échelle et le type ! connais pas
}
